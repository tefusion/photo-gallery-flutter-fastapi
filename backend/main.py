...
async def get_image_list(count: int, offset: int, sortMode: str = None, tag: str = None):
    sortModeType = convert_sort_mode(sortMode)
    return imageServer.get_image_list(offset, count, sortModeType, tag)

@app.get("/taglist/{count}/{offset}")
async def get_tag_list(count: int, offset: int, sortMode: str = None, tag: str = ""):
    sortModeType = convert_sort_mode(sortMode)
    return imageServer.get_tag_list(offset, count, sortModeType, tag)

@app.get("/autocomplete")
async def autocomplete(tagStart: str = ""):
    return imageServer.get_tags_starting_with_pattern(tagStart)
...
async def upload_images(title: str = Form(...), description: str = Form(...),
    ...
    await imageServer.upload_multiple_images(multiFileData)#would be better in imageServer, but since this is a 
